...
for_body(codegen_scope *s, node *tree)
{
  ...
  s = prev;
  s->simple_lambda = 0;
  if (s->mrb->irep[idx]->simple_lambda && 0) {
...
gen_call(codegen_scope *s, node *tree, mrb_sym name, int sp, int val)
{
  ...
  pop_n(n+1);
  {
    ...
    if (!noop && len == 1 && name[0] == '+')  {
      ...
      genop_send(s, MKOP_ABC(OP_DIV, cursp(), idx, n));
    }
    else if (!noop && len == 1 && name[0] == '<')  {
      genop_send(s, MKOP_ABC(OP_LT, cursp(), idx, n));
    }
    else if (!noop && len == 2 && name[0] == '<' && name[1] == '=')  {
      genop_send(s, MKOP_ABC(OP_LE, cursp(), idx, n));
    }
    else if (!noop && len == 1 && name[0] == '>')  {
      genop_send(s, MKOP_ABC(OP_GT, cursp(), idx, n));
    }
    else if (!noop && len == 2 && name[0] == '>' && name[1] == '=')  {
      genop_send(s, MKOP_ABC(OP_GE, cursp(), idx, n));
    }
    else if (!noop && len == 2 && name[0] == '=' && name[1] == '=')  {
      genop_send(s, MKOP_ABC(OP_EQ, cursp(), idx, n));
    }
    else {
      if (sendv) n = CALL_MAXARGS;

      if (blk > 0) {		   /* no block */
	genop_send(s, MKOP_ABC(OP_SEND, cursp(), idx, n));
      }
      else {
	genop_send(s, MKOP_ABC(OP_SENDB, cursp(), idx, n));
      }
    }
  }
  if (val) {
...
gen_vmassignment(codegen_scope *s, node *tree, int rhs, int val)
{
  ...
  if (tree->car) {              /* pre */
    ...
    n = 0;
    while (t) {
      genop_send(s, MKOP_ABC(OP_AREF, cursp(), rhs, n));
  ...
  if (t) {
    ...
    pop();
    genop_send(s, MKOP_ABC(OP_APOST, cursp(), n, post));
    n = 1;
    if (t->car) {               /* rest */
      gen_assignment(s, t->car, cursp(), NOVAL);
    }
...
{
  pop();
  genop_send(s, MKOP_ABC(OP_SEND, cursp(), new_msym(s, mrb_intern2(s->mrb, "intern", 6)), 0));
  push();
}
static void
...
codegen(codegen_scope *s, node *tree, int val)
{
      ...
      if (tree->car) {
        ...
        while (n2) {
          ...
          do {
            ...
            pop();
	  genop_send(s, MKOP_ABC(OP_SEND, cursp(), new_msym(s, mrb_intern2(s->mrb, "===", 3)), 1));
            tmp = new_label(s);
  ...
  case NODE_LAMBDA:
    {
      ...
      if (s->mrb->irep[s->idx + idx]->simple_lambda && 0) {
	/* no parent var access and child lambda */
      ...
      if ((intptr_t)t->car == NODE_ARRAY && nosplat(t->cdr)) {
        ...
        if (t) {
          ...
          if (t->car) {         /* rest (len - pre - post) */
            ...
            genop_send(s, MKOP_ABC(OP_ARRAY, cursp(), rhs+n, rn));
            gen_assignment(s, t->car, cursp(), NOVAL);
            n += rn;
          }
          if (t->cdr && t->cdr->car) {
        ...
        pop_n(len);
        if (val) {
          genop_send(s, MKOP_ABC(OP_ARRAY, rhs, rhs, len));
          push();
        }
      }
      else {
  ...
  case NODE_OP_ASGN:
    {
      ...
      if (len == 1 && name[0] == '+')  {
        ...
        genop_send(s, MKOP_ABC(OP_LE, cursp(), idx, 1));
      }
      else if (len == 1 && name[0] == '>')  {
        genop_send(s, MKOP_ABC(OP_GT, cursp(), idx, 1));
      }
      else if (len == 2 && name[0] == '>' && name[1] == '=')  {
        genop_send(s, MKOP_ABC(OP_GE, cursp(), idx, 1));
      }
      else {
        genop_send(s, MKOP_ABC(OP_SEND, cursp(), idx, 1));
      }
    }
    gen_assignment(s, tree->car, cursp(), val);
    break;
  ...
  case NODE_YIELD:
    {
      ...
      pop_n(n+1);
      if (sendv) n = CALL_MAXARGS;
      genop_send(s, MKOP_ABC(OP_SEND, cursp(), new_msym(s, mrb_intern2(s->mrb, "call", 4)), n));
      if (val) push();
    }
    break;
      ...
      default:
        {
          ...
          codegen(s, tree, VAL);
          pop(); pop();
          genop_send(s, MKOP_ABC(OP_SUB, cursp(), sym, 2));
        }
        break;
      }
    }
  ...
  case NODE_REGX:
    if (val) {
      ...
      genop_send(s, MKOP_ABC(OP_SEND, cursp(), sym, argc));
      mrb_gc_arena_restore(s->mrb, ai);
      push();
    }
    break;
  ...
  case NODE_ALIAS:
    {
      ...
      genop(s, MKOP_A(OP_LOADNIL, cursp()));
      pop_n(3);
      genop_send(s, MKOP_ABC(OP_SEND, cursp(), c, 2));
      if (val) {
        push();
      }
    }
  ...
  case NODE_UNDEF:
    {
      ...
      pop_n(num + 1);
      genop_send(s, MKOP_ABC(OP_SEND, cursp(), undef, num));
      if (val) {
        push();
      }
    }
